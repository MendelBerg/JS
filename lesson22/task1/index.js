...
removeBtn.addEventListener('click', () => {
  for (let i = 0; i < rectElems.length; i+=1) {
    ...
    rectElems[i].removeEventListener('click', eventsArr[i][1], false);
  }
});

attachBtn.addEventListener('click', () => {
  for (let i = 0; i < rectElems.length; i+=1) {
    rectElems[i].addEventListener('click', eventsArr[i][0]);
    rectElems[i].addEventListener('click', eventsArr[i][1], true);
  }
});
